...
// Default categories fallback if none provided
 ...
 */
export const AddTransactionModal = ({ isOpen, onClose, onSuccess, categories = [] }) => {
              ...
              <div className="relative" ref={categoryDropdownRef}>
                ...
                >
                  <span className="text-gray-900 flex items-center gap-2">
                    <span
                      ...
                      style={{ backgroundColor: categoryList.find(c => c.name === category)?.color || '#6b7280' }}
                    />
                    {category}
                  </span>
                  <ChevronDown
                ...
                {categoryDropdownOpen && (
                  <div className="absolute z-50 mt-1 w-full bg-white rounded-lg shadow-lg border border-gray-200 py-1 max-h-60 overflow-y-auto">
                    {categoryList.filter(c => c.name !== 'Income').map(cat => (
                      <button
                        key={cat.name}
                        type="button"
                        onClick={() => {
                        ...
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition-colors flex items-center gap-2 ${
                          cat.name === category ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-700'
                        }`}
                      >
                        <span
                          className="w-3 h-3 rounded-full flex-shrink-0"
